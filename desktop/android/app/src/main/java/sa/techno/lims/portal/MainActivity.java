...
public class MainActivity extends Activity {
 ...
 private void download(String url,String disposition,String mime){
  ...
  web.evaluateJavascript(script,value->{try{Object parsed=new org.json.JSONTokener(value).nextValue();if(!(parsed instanceof String))throw new Exception();pendingDownload=android.util.Base64.decode((String)parsed,android.util.Base64.DEFAULT);Intent save=new Intent(Intent.ACTION_CREATE_DOCUMENT).addCategory(Intent.CATEGORY_OPENABLE).setType(mime==null?"application/octet-stream":mime).putExtra(Intent.EXTRA_TITLE,mime!=null&&mime.contains("csv")?"TECHNO-template.csv":"TECHNO-download");startActivityForResult(save,102);}catch(Exception e){Toast.makeText(this,"Download failed. Open the system in your browser to download.",Toast.LENGTH_LONG).show();}});
 }
 private static final String HOME="https://osamababeker4-netizen.github.io/techno-lims/?release=10-9-4-techno-data-sync";
 private boolean trusted(Uri uri){return "https".equals(uri.getScheme())&&"osamababeker4-netizen.github.io".equals(uri.getHost())&&uri.getPath()!=null&&uri.getPath().startsWith("/techno-lims/");}
 ...
 @Override public void onCreate(Bundle state){
  ...
  Button home=new Button(this);home.setText("TECHNO LIMS");home.setOnClickListener(v->web.loadUrl(HOME));bar.addView(home);
  ...
  Button print=new Button(this);print.setText("⎙");print.setContentDescription("Print");print.setOnClickListener(v->{PrintManager manager=(PrintManager)getSystemService(PRINT_SERVICE);manager.print("TECHNO LIMS",web.createPrintDocumentAdapter("TECHNO LIMS"),new PrintAttributes.Builder().build());});bar.addView(print);
