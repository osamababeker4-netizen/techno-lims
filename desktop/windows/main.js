...
const HOME='https://osamababeker4-netizen.github.io/techno-lims/?release=10-9-4-techno-data-sync';
const trusted=url=>{try{const u=new URL(url);return u.origin==='https://osamababeker4-netizen.github.io'&&u.pathname.startsWith('/techno-lims/');}catch{return false;}};
function external(url){try{if(['https:','mailto:','tel:'].includes(new URL(url).protocol))shell.openExternal(url);}catch{}}
let main;
app.setName('TECHNO LIMS');
...
app.whenReady().then(()=>{
 session.defaultSession.setPermissionRequestHandler((webContents,permission,callback)=>{
  ...
  dialog.showMessageBox(main,{type:'question',message:'Allow TECHNO LIMS to use your location?',buttons:['Allow','Deny'],defaultId:1,cancelId:1}).then(result=>callback(result.response===0));
 });
 ...
 main=new BrowserWindow({show:!smoke,width:1380,height:900,minWidth:720,minHeight:520,title:'TECHNO LIMS 10.9.4',webPreferences:{nodeIntegration:false,contextIsolation:true,sandbox:true,webSecurity:true}});
 if(smoke){const timeout=setTimeout(()=>app.exit(2),45000);main.webContents.once('did-finish-load',async()=>{try{const ok=await main.webContents.executeJavaScript("typeof require==='undefined' && document.getElementById('loginForm')!==null");clearTimeout(timeout);console.log('TECHNO smoke test: '+ok);app.exit(ok?0:1);}catch{app.exit(1);}});}
